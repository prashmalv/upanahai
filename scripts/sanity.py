...
ok("this suite's votes are not counted", json.loads(body).get("mine", {}) == {}, body[:100])
ok("the survey reports it did not count us",
   json.loads(anon.go("/api/survey", {"question": "compare", "choice": "always"})[1]).get("counted") is False)
